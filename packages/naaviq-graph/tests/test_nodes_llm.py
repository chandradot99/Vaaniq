...
async def test_llm_response_adds_message():
    ...
    with patch("naaviq.graph.nodes.llm_response.get_llm", return_value=mock_llm):
...
async def test_condition_sets_route():
    ...
    with patch("naaviq.graph.nodes.condition.get_llm", return_value=mock_llm):
        node = ConditionNode(
            config={
...
def test_get_llm_openai():
    ...
    with patch("naaviq.graph.nodes.llm.ChatOpenAI") as mock_cls:
...
def test_get_llm_anthropic():
    from naaviq.graph.nodes.llm import get_llm
    with patch("naaviq.graph.nodes.llm.ChatAnthropic") as mock_cls:
