# naaviq-graph — Architecture Reference
...
`naaviq-graph` is the execution engine that turns a visual agent graph (stored as JSON) into a running LangGraph. It has no dependency on `naaviq-server` — it is a standalone Python package that any FastAPI app can use.
...
Storage Layer  graph_config JSONB column in agents table (naaviq-server)
...
Runtime Layer  GraphBuilder reads JSON → builds real LangGraph → executes
               ...
               Tools resolved from TOOL_REGISTRY (naaviq-tools)
...
Registered in `TOOL_REGISTRY` in `naaviq-tools`. Google Calendar, Gmail, HubSpot, Razorpay, Freshdesk, etc. User picks from a list in the `llm_response` or `run_tool` node config. Their API key (BYOK) is injected automatically.
...
```python
# naaviq/graph/nodes/llm_response.py
...
```python
# naaviq/graph/nodes/http_request.py
...
2. Register it in `NODE_REGISTRY` in `naaviq/graph/nodes/__init__.py`
...
Tools are registered in `TOOL_REGISTRY` (in `naaviq-tools`). When an `llm_response` node has `"tools": ["google_calendar_create", "crm_create_lead"]`:
...
**Naaviq is the first open-source visual LangGraph builder for voice/chat/WhatsApp agents.** All existing tools are either text-only, closed source, or debuggers rather than builders.

---

## What NOT To Do

- **Don't import from `naaviq-server`** — this package has no server dependency
