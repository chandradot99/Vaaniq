...
naaviq-core's SessionState is a plain TypedDict with no external deps.
This module extends it with LangGraph reducers so the graph engine
can merge partial updates correctly.

Why separate from naaviq-core:
  naaviq-core must stay dependency-free (it's the contract used by all
