...
def _wrap_as_lc_tool(naaviq_tool: Any, org_keys: dict) -> StructuredTool:
    """Convert a naaviq BaseTool into a LangChain StructuredTool."""
    _tool = naaviq_tool
    _keys = org_keys
...
class LLMResponseNode(BaseNode):
    async def __call__(self, state: GraphSessionState) -> dict:
        try:
            from naaviq.tools.registry import TOOL_REGISTRY  # deferred to avoid circular import
        except ImportError:
            TOOL_REGISTRY = {}  # naaviq-tools not installed (isolated test env)
