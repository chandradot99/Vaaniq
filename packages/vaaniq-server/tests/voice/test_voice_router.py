...
def test_add_request_rejects_non_e164():
    ...
    from vaaniq.server.voice.schemas import AddPhoneNumberRequest
...
def test_outbound_request_rejects_non_e164():
    from pydantic import ValidationError
    from vaaniq.server.voice.schemas import OutboundCallRequest
    with pytest.raises(ValidationError):
...
async def test_list_phone_numbers_empty(client):
    with patch("vaaniq.server.voice.router.VoiceService") as MockSvc:
...
async def test_add_phone_number_409_on_duplicate(client):
    from vaaniq.server.voice.exceptions import PhoneNumberAlreadyExists
...
async def test_remove_phone_number_404(client):
    from vaaniq.server.voice.exceptions import PhoneNumberNotFound
...
async def test_outbound_call_success(client):
    from vaaniq.server.voice.schemas import OutboundCallResponse
...
async def test_outbound_call_missing_twilio_creds(client):
    from vaaniq.server.voice.exceptions import TwilioCredentialsMissing
...
async def test_outbound_call_twilio_error_returns_502(client):
    from vaaniq.server.voice.exceptions import OutboundCallFailed
