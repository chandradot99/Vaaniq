...
Voice pipeline package for Vaaniq. Handles phone calls and browser WebRTC via **LiveKit Agents SDK**. Sits above `vaaniq-core` and `vaaniq-graph` in the dependency tree — the LangGraph agent is channel-agnostic; only the I/O layer differs per channel.
...
1. Caller dials Twilio number
       → Twilio POST /webhooks/twilio/voice/inbound  (vaaniq-voice-server)
...
2. Twilio connects to LiveKit SIP
       ...
       → creates a room, dispatches job to vaaniq-voice worker
...
```
1. POST /v1/voice/calls/outbound  { agent_id, from_number, to_number }  (vaaniq-server)
