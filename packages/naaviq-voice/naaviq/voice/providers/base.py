...
class ModelInfo:
    ...
    """ISO 639-1 or BCP-47 language codes this model supports. ['*'] means all languages."""
    is_default: bool = False
    streaming: bool = True
    """Whether this model supports real-time streaming. Non-streaming models cannot be used for live voice calls."""
    category: str | None = None
    """Optional grouping label shown in the UI, e.g. 'Current Generation', 'Legacy'."""
...
class STTProviderBase(ABC):
    ...
    Optionally override:
      ...
      - languages        → BCP-47 codes this provider supports; ['*'] means all languages
    """
...
class TTSProviderBase(ABC):
    ...
    Optionally override:
      ...
      - languages                         → BCP-47 codes this provider supports; ['*'] means all
    """
    ...
    async def synthesize_preview(
        ...
        """
        Synthesize `text` using the given voice config and return (audio_bytes, mime_type).

        `config` keys mirror VoiceConfig fields:
          tts_model, tts_voice_id, tts_speed, tts_emotion, tts_stability, tts_style, tts_instructions

        Returns None if preview is not supported by this provider.
        Must never raise — catch all exceptions internally and return None on failure.
        """
        return None
