...
class PhoneNumber(Base):
    ...
    __table_args__ = (
        ...
        # Partial unique index — friendly_name must be unique per org (when non-null and active).
        # NULL names are always allowed (multiple pipelines can have no name).
        Index(
            "uq_phone_numbers_org_name_active",
            "org_id",
            "friendly_name",
            unique=True,
            postgresql_where=text("deleted_at IS NULL AND friendly_name IS NOT NULL"),
        ),
    )
