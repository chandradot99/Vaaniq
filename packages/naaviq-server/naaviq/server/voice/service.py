...
class VoiceService:
    ...
    async def get_phone_number(self, org_id: str, number_id: str) -> PhoneNumberResponse:
        pn = await self._phone_repo.get_by_id(number_id)
        if not pn:
    ...
    async def update_phone_number_name(
        self, org_id: str, number_id: str, body: UpdatePhoneNumberNameRequest
    ) -> PhoneNumberResponse:
        pn = await self._get_own_number(org_id, number_id)
        if body.friendly_name:
            existing = await self._phone_repo.get_by_friendly_name(org_id, body.friendly_name)
            if existing and existing.id != number_id:
                raise PhoneNumberNameConflict(body.friendly_name)
        await self._phone_repo.update_friendly_name(number_id, body.friendly_name)
        await self.db.commit()
        await self.db.refresh(pn)
        log.info("phone_number_renamed", org_id=org_id, number_id=number_id, name=body.friendly_name)
        return _to_phone_response(pn)
    ...
    async def initiate_outbound(
        ...
        LiveKit production flow:
          ...
          2. Dispatch the naaviq-voice agent to the room — worker joins with correct metadata.
        ...
        """
        from naaviq.server.core.config import settings as _settings
