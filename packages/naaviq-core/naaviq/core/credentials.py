...
naaviq-core defines the interface only — no external dependencies, no storage logic.

Implementations live in:
    naaviq-tools   → EnvCredentialStore   (env vars, for standalone dev use)
    naaviq-server  → PostgresCredentialStore (encrypted DB, multi-tenant)
