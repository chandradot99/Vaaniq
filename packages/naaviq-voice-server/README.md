...
Standalone voice server for Naaviq. Handles telephony webhooks (Twilio, Telnyx, Vonage) and runs the LiveKit voice agent worker as a single process.
...
Signature verification uses `X-Twilio-Signature` HMAC-SHA1 (automatic via naaviq-server).
...
All webhooks are on `naaviq-voice-server` (port 8001). No `/v1/` prefix — providers control these URLs.
...
`fly.toml` points to `packages/naaviq-voice-server/Dockerfile`. The `CMD` runs uvicorn which starts the LiveKit worker internally via the lifespan hook.
